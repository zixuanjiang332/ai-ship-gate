...
async function maybeExplainWithAi(options) {
  ...
  try {
    const response = await fetchImpl(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You summarize deterministic release gate findings. Do not change the verdict. Keep the answer under 120 words."
          },
          {
            role: "user",
    ...
    return data.choices?.[0]?.message?.content?.trim() || void 0;
  } catch {
    return void 0;
  } finally {
...
function validateConfig(config) {
  ...
  if (config.failOn !== void 0 && config.failOn !== "warn" && config.failOn !== "fail") {
  ...
  if (config.ai !== void 0) {
    ...
    if (config.ai.enabled !== void 0 && typeof config.ai.enabled !== "boolean") {
...
function shouldExitWithFailure(verdict, failOn) {
...
function parseChangedFiles(nameStatus, patch) {
...
function extractPatchForPath(patch, path, status) {
...
function diffSectionMatchesPath(section, path, status) {
...
function diffHeaderMatchesSamePath(header, path) {
...
function detectMetadata(lsFilesOutput) {
  const files = new Set(lsFilesOutput.split(/\r?\n/).filter(Boolean));
  const hasAny = (...paths) => paths.some((path) => files.has(path));
...
async function resolveDefaultBase(exec, cwd) {
...
var schemaUri = "https://docs.oasis-open.org/sarif/sarif/v2.1.0/os/schemas/sarif-schema-2.1.0.json";
var informationUri = "https://github.com/zixuanjiang332/releaseguard-ai";
...
function renderTerminal(report, options = {}) {
  ...
  for (const finding of report.findings) {
    lines.push(formatFinding2(finding), "");
  }
  return lines.join("\n");
}
...
function colorFor(verdict) {
  if (verdict === "fail") return import_picocolors.default.red;
  if (verdict === "warn") return import_picocolors.default.yellow;
  return import_picocolors.default.green;
}
function sanitizeTerminalText(value) {
  return value.replaceAll(/\u001B\][^\u0007]*(?:\u0007|\u001B\\)/g, "").replaceAll(/\u001B\[[0-?]*[ -/]*[@-~]/g, "").replaceAll(/[\r\n]+/g, " ").replaceAll(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]/g, "");
}
...
function isRealEnvFilePath(path) {
