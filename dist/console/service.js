...
export async function runConsoleCheck(request, runCheck = defaultRunCheck) {
    ...
    const counts = result.report.findings.reduce((summary, finding) => {
...
function buildConsoleFileEntries(changedFiles, findings) {
    const matchedFindingIds = new Map();
    for (const finding of findings) {
        for (const file of finding.files) {
            const ids = matchedFindingIds.get(file) ?? new Set();
            ids.add(finding.id);
    ...
    return changedFiles
        ...
        matchedFindingIds: [...(matchedFindingIds.get(file.path) ?? new Set())],
    }));
}
function toSnippet(patch) {
