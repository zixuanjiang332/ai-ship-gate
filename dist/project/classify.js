...
export function isCiOrDeployPath(path) {
    const normalized = normalize(path);
...
export function patchAddsFocusedOrSkippedTest(patch) {
    return addedLines(patch).some((line) => /\.(only|skip)\s*\(|\b(xit|fit)\s*\(/.test(line));
}
export function patchAddsEnvUsage(patch) {
    return addedLines(patch).some((line) => /(process\.env\.[A-Z0-9_]+|process\.env\[['"][A-Z0-9_]+['"]\]|import\.meta\.env\.[A-Z0-9_]+|os\.environ\[['"][A-Z0-9_]+['"]\]|getenv\(['"][A-Z0-9_]+['"]\))/.test(line));
}
export function patchContainsSecret(patch) {
