...
function resolveAnchorInFile(finding: Finding, changedFile: ChangedFile): ReviewAnchor | undefined {
...
function firstAddedLine(patch: string): AddedLine | undefined {
  return addedLinesWithNumbers(patch)[0];
}

function firstMatchingAddedLine(patch: string, predicate: (line: string) => boolean): AddedLine | undefined {
  return addedLinesWithNumbers(patch).find((line) => predicate(line.content));
}

function addedLinesWithNumbers(patch: string): AddedLine[] {
...
function hasFindingFile(finding: Finding, path: string): boolean {
...
function toAnchor(file: string, addedLine: AddedLine | undefined): ReviewAnchor | undefined {
  if (!addedLine) {
    return undefined;
  }
